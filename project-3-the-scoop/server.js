// Packages requried to support saving and loading the database to/from YAML
// files: js-yaml and fs.
const yaml = require('js-yaml');
const fs = require('fs');
const dbFile = 'state/database.yaml';
...
// representing the database saveDatabase.
function loadDatabase() {
  try {
    return yaml.safeLoad(fs.readFileSync(dbFile, 'utf8'));
  } catch (e) {
    console.log(e);
  }
}
...
function saveDatabase() {
  try {
    fs.writeFileSync(dbFile, yaml.safeDump(database, 'utf8'));
  } catch (e) {
    console.log(e);
  }
...
// Creates new comment and adds it to database, returns a 201 response
// with comment on comment property of response body. If body isn't
// supplied, user with supplied username doesn't exist, or article with
...
function createComment(url, request) {
  ...
  switch(true) {
    case !request.body:
    case !request.body.comment:
    case !request.body.comment.body:
    case !request.body.comment.username:
    case !request.body.comment.articleId:
    case !database.users[request.body.comment.username]:
    case !database.articles[request.body.comment.articleId]:
      response.status = 400;
      break;
    default:
      const comment = {
      ...
      response.body = {comment: database.comments[comment.id]};
      response.status = 201;
  }
...
// property of request body. Updates body of corresponding comment in
// database, returns a 200 response with the updated comment on comment
// property of the response body. If comment with given ID does not exist,
// returns 404 response. If no ID or updated comment is supplied, returns
// 200 response.
function updateComment(url, request) {
  ...
  switch(true) {
    ...
    case !request.body.comment.body:
      response.status = 400;
      break;
    case !database.comments[commentId]:
      response.status = 404;
      break;
    default:
...
// Receives comment ID from URL parameter. Deletes comment from database
// and removes all references to its ID from corresponding user and article
// models, returns 204 response. If no ID is supplied or comment with
// supplied ID doesn't exist, returns 400 response.
function deleteComment(url, request) {
  ...
  switch(true) {
    ...
    default:
      ...
      database.users[username].commentIds =
        database.users[username].commentIds.filter(id => id != commentId);
      database.articles[articleId].commentIds =
        database.articles[articleId].commentIds.filter(id => id != commentId);
...
// Receives comment ID from URL parameter and username from username
// property of request body. Adds supplied username to upvotedBy of
// corresponding comment if user hasn't already upvoted the comment,
// removes username from downvotedBy if that user had previously downvoted
// the comment, returns 200 response with comment on comment property of
// response body. If no ID is supplied, comment with supplied ID doesn't
// exist, or user with supplied username doesn't exist, returns 400
// response.
function upvoteComment(url, request) {
  ...
  switch(true) {
    ...
    case !database.comments[commentId]:
    case !database.users[request.body.username]:
      response.status = 400;
      break;
    default:
      let username = request.body.username;
      database.comments[commentId].downvotedBy =
        database.comments[commentId].downvotedBy.filter(name => name != username);
      if(database.comments[commentId].upvotedBy.indexOf(username) === -1) {
...
// Receives comment ID from URL parameter and username from username
// property of request body. Adds supplied username to downvotedBy of
// corresponding comment if user hasn't already downvoted the comment,
// remove username from upvotedBy if that user had previously upvoted the
// comment, returns 200 response with comment on comment property of
// response body. If no ID is supplied, comment with supplied ID doesn't
// exist, or user with supplied username doesn't exist, returns 400
// response.
function downvoteComment(url, request) {
    ...
    switch(true) {
      ...
      default:
        let username = request.body.username;
        database.comments[commentId].upvotedBy =
          database.comments[commentId].upvotedBy.filter(name => name != username);
        if(database.comments[commentId].downvotedBy.indexOf(username) === -1) {
